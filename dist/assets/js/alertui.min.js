...
;(function(context,namespace){'use strict';var alertuiWin=window[namespace];var proto={addClass:function(el,cls){var elClass=el.className;var blank=(elClass!=='')?' ':'';var added=elClass+blank+cls;el.className=added;},removeClass:function(el,cls){var elClass=' '+el.className+' ';elClass=elClass.replace(/(\s+)/gi,' ');var removed=elClass.replace(' '+cls+' ',' ');removed=removed.replace(/(^\s+)|(\s+$)/g,'');el.className=removed;},addEvent:function(el,type,func){if(el.addEventListener){el.addEventListener(type,func,false);}else if(el.attachEvent){el.attachEvent('on'+type,func);}else{el['on'+type]=func;}},dispatchEvent:function(el,etype){if(el.fireEvent){el.fireEvent('on'+etype);}else{var evObj=document.createEvent('Events');evObj.initEvent(etype,true,false);el.dispatchEvent(evObj);}},removeEvent:function(el,type,func){if(el.removeEventListener){el.removeEventListener(type,func,false);}else if(el.detachEvent){el.detachEvent('on'+type,func);}else{delete el['on'+type];}},removeElement:function(el){(el&&el.parentNode)&&el.parentNode.removeChild(el);}};function Alertui(){var same,alertConfig,noteConfig,btOkv='Ok',btCancelV='Cancel',callTime=500,noteTime=4300,block=false,_$generate_;_$generate_={createModal:function(){var elModal=document.createElement('div');proto.addClass(elModal,'altui-modal');return elModal;},blockBody:function(flg){var elBdy;elBdy=document.body;flg?elBdy.style.overflow='hidden':elBdy.style.overflow='';},promptResponse:function(el,call,alertEl,elDataIn){var reqValue;proto.addEvent(el,'click',function(event){_$generate_.blockBody(false);if(event.target.matches('.altui-modal, .alt-btn, .altui-close')){reqValue=elDataIn.value;if(block)return false;if(reqValue!==''){block=true;el.setAttribute('disabled','disabled');proto.addClass(alertEl,'alert-close');setTimeout(function(){proto.removeElement(alertEl);if(call!==undefined)
call(reqValue);block=false;},callTime);}else
elDataIn.focus();}});},alertResponse:function(el,call,alertEl){proto.addEvent(el,'click',function(event){_$generate_.blockBody(false);if(event.target.matches('.altui-modal, .alt-btn, .altui-close')){if(block)return false;block=true;proto.addClass(alertEl,'alert-close');setTimeout(function(){proto.removeElement(alertEl);if(call!==undefined)call();block=false;},callTime);}});},loadResponse:function(call,alertEl){if(block)return false;block=true;if(call!==undefined){block=false;call(function(){_$generate_.blockBody(false);proto.addClass(alertEl,'alert-close');setTimeout(function(){proto.removeElement(alertEl);},callTime);},alertEl);}},noteResponse:function(el,call,elCase){proto.addEvent(el,'click',function(){proto.addClass(el,'note-close');setTimeout(function(){proto.removeElement(el);if(call!==undefined)call();if(elCase.childNodes.length===0)
...
proto.removeElement(elCase);},callTime);},noteTime);},message:function(altOpts){var altUi=document.createElement('div'),altBox=document.createElement('div'),altCm=document.createElement('div'),btClose=document.createElement('button'),altHd=document.createElement('div'),altBdy=document.createElement('div'),altFt=document.createElement('div'),btnOk=document.createElement('button'),btnCancel=document.createElement('button'),modal,dataIn;this.blockBody(true);btClose.innerHTML='×';if(typeof altOpts.opt!=="object")
...
function show(same){var mId,mdEl;mId=same.dataset.modal;mdEl=document.getElementById(mId);proto.addClass(mdEl,'show');}
function close(mdl){if(event.target.matches('.altui-modal, .altui-close')){mdl.classList.add('alert-close');setTimeout(function(){mdl.classList.remove('show','alert-close');},callTime);}}
function setActions(mdl){var cBt,mEl,i;cBt=mdl.getElementsByClassName('altui-close');mEl=mdl.getElementsByClassName('altui-modal')[0];for(i=0;i<cBt.length;i++){proto.addEvent(cBt[i],'click',function(){close(mdl);});}
proto.addEvent(mEl,'click',function(event){if(event.target.matches('.altui-modal')){close(mdl);}});}
